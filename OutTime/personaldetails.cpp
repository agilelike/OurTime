    ...
    ui(new Ui::personalDetails)
{
    ...
    QColor backColor = qRgb(255,255,255);    //画布初始化背景色白色
    image1 = QImage(220,150,QImage::Format_RGB32);  //画布的初始化大小设/颜色
    image1.fill(backColor);//对画布进行填充
    PaintImage1();

    image2 = QImage(220,150,QImage::Format_RGB32);  //画布的初始化大小设/颜色
    image2.fill(backColor);//对画布进行填充
    PaintImage2();
...
void personalDetails::PaintImage1()
{
    ...
    int pointx=30,pointy=130;//确定坐标轴起点坐标
    int width=200-pointx,height=120;//确定坐标轴宽度跟高度
    ...
    //y轴刻度线
    double maxStep=(double)max/10;//y轴刻度间隔需根据最大值来表示
    for(int i=0;i<7;i++)
    {
