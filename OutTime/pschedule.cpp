...
pSchedule::pSchedule(QDate day,bool person)
{
    ...
    if(person==0)
    {
        ...
        while(query.next())
        {
            Schedule sche;
            sche.state=0;
            sche.t=QDate::fromString(query.value(0).toString(),"yyyy-MM-dd");
    ...
    else if(person==1)
    {
        ...
        str = QString("select * from task where teamID = '%1' and date_format(deadline,'%Y-%m-%d') = '%2'").arg(user->getTeamid()).arg(date.toString("yyyy-MM-dd"));
        query.exec(str);
        while(query.next())
        {
            Schedule sche;
            ...
            QDateTime t=QDateTime::fromString(query.value(2).toString(),"yyyy-MM-ddTHH:mm:ss");
            sche.t=t.date();
            sche.end=t.time();
            sche.name=query.value(3).toString();
            s.append(sche);
